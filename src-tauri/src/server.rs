...
pub struct AppState {
    ...
    pub ws: Arc<WsHub>,
    /// Agent runtime: provider/tool loop shared with the desktop head. Server
    /// turns are owner-scoped via `OwnedWsAgentSink`.
...
async fn dispatch(st: &AppState, actor: &User, cmd: &str, args: Value) -> Result<Value, String> {
    ...
    match cmd {
        ...
        // ── Agent runtime (owner-scoped) ────────────────────────────────────
        // The authenticated `User` is the only source of the owner id; any
        // `ownerId` inside the args is rejected by `StartTurnRequest`
        // (`deny_unknown_fields`).
        "agent_start_turn" => {
            let request: crate::agent::StartTurnRequest =
                serde_json::from_value(args.get("request").cloned().unwrap_or(Value::Null))
                    .map_err(|e| format!("invalid agent request: {e}"))?;
            let actor_ctx = crate::agent::ActorContext {
                owner_id: actor.id.to_string(),
            };
            let sink: Arc<dyn crate::agent::AgentEventSink> = Arc::new(
                crate::server_ws::OwnedWsAgentSink::new(st.ws.clone(), actor.id.to_string()),
            );
            let handle = st
                .agent
                .start_turn(actor_ctx, request, sink)
                .await
                .map_err(estr)?;
            serde_json::to_value(handle).map_err(estr)
        }
        "agent_respond" => {
            let turn_id = require(&args, "turnId")?.to_string();
            let response: crate::agent::ClientTurnResponse =
                serde_json::from_value(args.get("response").cloned().unwrap_or(Value::Null))
                    .map_err(|e| format!("invalid agent response: {e}"))?;
            let actor_ctx = crate::agent::ActorContext {
                owner_id: actor.id.to_string(),
            };
            st.agent
                .respond(actor_ctx, crate::agent::TurnHandle { turn_id }, response)
                .await
                .map_err(estr)?;
            Ok(Value::Null)
        }
        "agent_cancel" => {
            ...
            st.agent
                .cancel(actor_ctx, crate::agent::TurnHandle { turn_id })
                .await
                .map_err(estr)?;
            Ok(Value::Null)
        }
...
async fn ws_handler(State(st): State<AppState>, headers: HeaderMap, ws: WebSocketUpgrade) -> Response {
    ...
    let Some(user) = resolve_session(&st, &token) else {
        return (StatusCode::UNAUTHORIZED, "未登录").into_response();
    };
    ...
    ws.on_upgrade(move |socket| handle_ws(socket, st, token, owner_id))
}
...
async fn handle_ws(socket: WebSocket, st: AppState, token: String, owner_id: String) {
