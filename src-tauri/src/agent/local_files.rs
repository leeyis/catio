...
pub fn tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec { name: WRITE_FILE.into(), description: "Save UTF-8 text/Markdown to the user's LOCAL workspace (not the SSH server), at most 1 MiB. Only use when the user explicitly requests creating, saving, exporting or updating a local file/document. Ordinary replies, summaries and intermediate progress messages must stay in chat unless the user requests a file. Default create never overwrites. For replace, read first and supply expectedVersion. Only claim saved after success.".into(), input_schema: json!({"type":"object","properties":{"path":{"type":"string"},"content":{"type":"string"},"mode":{"type":"string","enum":["create","replace"]},"expectedVersion":{"type":"string","description":"Version returned by reading the file; required for replace"}},"required":["path","content"],"additionalProperties":false}) },
    ]
}
