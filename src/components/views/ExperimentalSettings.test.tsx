...
describe('experimental feature gate', () => {
  beforeEach(() => {
    ...
    mocks.status.mockResolvedValue({ configured: true, canConfigure: true }); mocks.lock.mockResolvedValue(undefined)
  })
  ...
  it('requires matching initial passphrases and prevents non-admin setup', async () => {
    mocks.status.mockResolvedValue({ configured: false, canConfigure: true })
    ...
    view.unmount(); mocks.status.mockResolvedValue({ configured: false, canConfigure: false })
    renderSettings(); await act(async () => {})
    fireEvent.click(screen.getByRole('switch'))
