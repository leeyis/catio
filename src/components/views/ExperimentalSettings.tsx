...
export function ExperimentalSettings() {
  ...
  function cancel() { generation.current++; setPrompt(false); setPassphrase(''); setConfirmation(''); setBusy(false); setError('') }
  ...
  return <section>
    ...
    {prompt && <form role="dialog" aria-modal="false" aria-label={t(status?.configured ? 'optical.unlockTitle' : 'optical.setupTitle')}
      ...
      <div className="row gap8" style={{ justifyContent: 'flex-end' }}>
        ...
        <Btn type="submit" disabled={busy || !passphrase}>{t(busy ? 'optical.unlocking' : 'optical.enable')}</Btn>
      </div>
    </form>}
