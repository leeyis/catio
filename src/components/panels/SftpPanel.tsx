...
export function SftpPanel({ onClose, conn, sessionId }: SftpPanelProps) {
  ...
  // Use mock as initial value so demo mode is pixel-identical on first render
  const [sftp, setSftp] = useState<Sftp>(D.sftp)
  const [path, setPath] = useState<string>(D.sftp.path)
  ...
  return (
    <PanelShell icon="folder" title={`SFTP · ${conn ? conn.name : 'prod-web-01'}`} sub={sftp.path} onClose={onClose}
