...
export function DbWorkbench({ conn, density }: DbWorkbenchProps) {
  ...
  // Resolve capabilities: find the first active connection whose profileId matches conn.id.
  // Falls back to ALL_ENABLED when not connected (mock/demo / non-Tauri path).
  const caps: DbCapabilities = useMemo(() => {
    const active = listActiveDbConnections().find(a => a.profileId === conn.id)
    return active ? active.capabilities : ALL_ENABLED
  }, [conn.id])
        ...
        {obj.type === 'table' && (
          <>
            ...
            <div className="grow" style={{ minHeight: 0 }}>
              {effectiveTableTab === 'data' && <DataGrid
